...
public class MenuClienteView extends JFrame {
	...
	public MenuClienteView(Cliente cliente) {
		...
		*/
		...
		JLabel Ola = new JLabel("Bem vindo, " + this.cliente_logado.getNome_usuario());
		...
		JToggleButton Saldo = new JToggleButton("Mostrar");
		...
		Saldo.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));	
		panel.add(Saldo);
		...
		Saldo.addActionListener(e -> {
	        ...
	        if (a) {
	            ...
	            if (senha.equals(this.cliente_logado.getSenha())) {  
	            	try {
		...
		Limite.addActionListener(e -> {
			Conta conta;
			try {
				conta = ContaDAO.findByIdCliente(this.cliente_logado.getId_usuario());
				if(conta instanceof ContaCorrente) {
					ContaCorrente cc = (ContaCorrente) conta;
					JOptionPane.showMessageDialog(this, "R$ " + cc.getLimite(), "Limite disponível", JOptionPane.INFORMATION_MESSAGE);
				}else {
					JOptionPane.showMessageDialog(this, "Não há opção de limite disponível", "Limite disponível", JOptionPane.INFORMATION_MESSAGE);
				}
			} catch (Exception e1) {
